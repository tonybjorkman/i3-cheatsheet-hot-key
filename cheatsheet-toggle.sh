...
primary_terminal=Gnome-terminal
popup_terminal=xfce4-terminal
...
if [ $niceclass == $primary_terminal ] && [[ $nicewm_name != "Cheatsheet for"* ]] 
then
    ...
    #echo 'termPID:'$terminalPID >> $debugfile
    ...
    $popup_terminal -T 'Cheatsheet for '$shell_process -x vim $cheatsheet_path
...
then
    ...
    touch $cheatsheet_path
    $popup_terminal -T 'Cheatsheet for '$niceclass -x vim $cheatsheet_path
fi
